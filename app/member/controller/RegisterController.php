...
class RegisterController extends HomeBaseController
{
    ...
    */
    public function lsg_doRegister(){
        ...
        $errorinfo = (new MemberValidate())->goCheck('lsg_doRegister', $data);
...
//        用户输入验证码
    ...
    public function SmMobiles(){
        $data = $this->request->param();
        if (!isset($data['mobile'])) $this->ajaxReturn(['msg' => '手机号必须', 'status' => -1]);
...
//                $message = explode(':', explode(',', $data)[1])[1];
    ...
    /*
        ...
        password 用户密码
    */
    public function lsg_doLogin(){

        //接受全部数据
        $data = $this->request->param();

        $errorinfo = (new MemberValidate())->goCheck('lsg_doLogin', $data);
        if($errorinfo !== true) $this->ajaxResult(-1, $errorinfo['msg'], $errorinfo['data']);
...
//            var_dump($user); die;
            //调用user模型
            $usermodel = new MemberModel();
            $log  = $usermodel->lsg_doMobile($user);
            switch ($log['status']) {
                case 0:
                    $this->ajaxReturn(['msg'=>'登录成功','status'=>1,'token'=>$log['token']]);
                    break;
                case 1:
                    $this->ajaxReturn(['msg'=>'您输入的密码存在错误','status'=>-1]);
                    break;
                case 2:
                    $this->ajaxReturn(['msg'=>'您输入的账号不存在','status'=>-1]);
                    break;
                case 3:
                    $this->ajaxReturn(['msg'=>'账号被禁止访问系统','status'=>-1]);
                    break;
                default :
                    $this->ajaxReturn(['msg'=>'请求错误','status'=>-1]);
            }
        }else{
            $this->ajaxReturn(['msg' => '手机号码有误,请重新输入!','status' => -1]);
        }
    }
